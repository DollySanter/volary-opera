...
var (
	...
	RPCGlobalGasCapFlag = cli.Uint64Flag{
		Name:  "rpc.gascap",
		Usage: "Sets a cap on gas that can be used in volary_call/estimateGas (0=infinite)",
		Value: gossip.DefaultConfig(cachescale.Identity).RPCGasCap,
	}
	RPCGlobalTxFeeCapFlag = cli.Float64Flag{
		Name:  "rpc.txfeecap",
		Usage: "Sets a cap on transaction fee (in volary) that can be sent via the RPC APIs (0 = no cap)",
...
func mayGetGenesisStore(ctx *cli.Context) *genesisstore.Store {
	...
	case ctx.GlobalIsSet(FakeNetFlag.Name):
		...
		return makefakegenesis.FakeGenesisStore(num, futils.ToVolary(1000000000), futils.ToVolary(5000000))
