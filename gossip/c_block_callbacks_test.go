...
func TestConsensusCallback(t *testing.T) {
	...
	for n := uint64(0); n < rounds; n++ {
		...
		for i := idx.Validator(0); i < validatorsNum; i++ {
			...
			to := 0
			txs[i] = env.Transfer(idx.ValidatorID(from+1), idx.ValidatorID(to+1), utils.ToVolary(100))
		}
		tm := sameEpoch
		if n%10 == 0 {
			tm = nextEpoch
		...
		// balance movements
		balances[0].Add(balances[0], utils.ToVolary(200))
		balances[1].Sub(balances[1], utils.ToVolary(100))
		balances[2].Sub(balances[2], utils.ToVolary(100))
	}
